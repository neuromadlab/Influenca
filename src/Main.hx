...
//%%%%%%%%%%%%% VARIABLEN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	// Set up Crashdumper vars
	public var crashdumper_id:String;
	public var crashDumper:CrashDumper;
	...
	public var export_litedb_info:InfoText;
	...
	public var button_db_export:SimpleButton;
...
//%%%%%%%%%%BUTTONS%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	public function onClick_pw_reset_code (event: MouseEvent):Void {
		...
		if (database_availability == true && pw_rest_mail_input.length > 0) {
			...
			var database_accessible = DatabaseSync.CheckPasswordReset(pw_rest_mail_input.text);
			
			// Check if connection to the database could be established, if not display info field
			if (database_accessible == false) {
				
				// Display info text field: Database currently not accessible
				#if mobile
				pw_reset_code_info_text = new InfoText (language.registration_text.inf_text5,1);
				#elseif desktop
				pw_reset_code_info_text = new InfoText (language.registration_text.inf_text6,1);
				#end
	...
	public function onClick_pw_reset_submit (event: MouseEvent):Void {
		...
		if (database_availability == true && pw_rest_mail_input.length > 0 && pw_rest_code_input.length > 0 && pw_reset_passw.length > 0) {
			...
			} else if (transaction_finished == 99) {
				
				// Database is not available at the moment
				#if mobile
				info_text_str = language.registration_text.inf_text5;
				#elseif desktop
				info_text_str = language.registration_text.inf_text6;
				#end
	...
	public function onClick5 (event: MouseEvent):Void {
		...
		if (database_availability == true) {
			...
			if (transaction_finished == true){
			...
			} else if (transaction_finished == false){
				// Display info text field: Database currently not accessible
				#if mobile
				logout_inet_info = new InfoText (language.registration_text.inf_text5,1);
				#elseif desktop
				logout_inet_info = new InfoText (language.registration_text.inf_text6,1);
				#end
				...
				preferences_screen.addChild(logout_inet_info);
			}
	...
	public function toggleMessageLogoutInet(event: MouseEvent):Void {
		preferences_screen.removeChild(logout_inet_info);
				...
				// If internet is connected, check if input fields are filled 
						if (mailaddress.length > 0 && selectedpw.length > 0) {
							...
							if (mail_availability == 0){ // mail address is not registered yet
								...
								// Activate crashdumper after ID is set and appdata exists
								crashdumper_id = SessionData.generateID("neuroMADLAB_Influenca_");
								#if desktop
									var crashdump_path = Path.join([System.documentsDirectory, "Crash_Logs"]);
								#elseif mobile
									var androidexternalstorage = AndroidExternalStorage.getAndroidStoragePath();		
									var crashdump_path = Path.join([androidexternalstorage, "Crash_Logs"]);
								#end
								if (!FileSystem.exists(crashdump_path)) {
									 FileSystem.createDirectory(crashdump_path);
								}
								crashDumper = new CrashDumper(crashdumper_id,crashdump_path);
								//Include own config
								...
								crashDumper.session.files.set("Appdata.json", appdata_json);
							...
							} else if (mail_availability == 99){
								
								// Display info text field: Database currently not accessible
								#if mobile
								reg_mail_info = new InfoText (language.registration_text.inf_text5,1);
								#elseif desktop
								reg_mail_info = new InfoText (language.registration_text.inf_text6,1);
								#end
		...
		if (database_availability == true) {
				if (username.length > 0 && passw.length > 0) {
					...
					if (mail_availability == 2){ // mail address is registered and password fits -> ID, keycode & keycode entered state are already retrieved from database
						...
						// (Re-)Set Crashdumper after login
						crashdumper_id = SessionData.generateID("neuroMADLAB_Influenca_");
						#if desktop
							var crashdump_path = Path.join([System.documentsDirectory, "Crash_Logs"]);
						#elseif mobile
						...
						// Display info text field: Database currently not accessible
						#if mobile
						log_password_info = new InfoText (language.registration_text.inf_text5,1);
						#elseif desktop
						log_password_info = new InfoText (language.registration_text.inf_text6,1);
						#end
						var log_password_info_button = log_password_info.getChildAt(1);
						log_password_info_button.addEventListener(MouseEvent.CLICK, toggleMessageLogPassw,false,0,true);
						login_screen.addChild(log_password_info);
	...
	public function log_and_reg(){
		...
		// Add title at the end to prevent it from being cut off
	...
	public function drawPreferencesScreen(){
		...
		// Export databases
		button_db_export = Button.drawButton(language.button_texts.export_button_text, Std.int(NOMINAL_WIDTH / 2), 600, "menu");	
		//Logout
		button3 = Button.drawButton(language.menu_texts.logout, Std.int(NOMINAL_WIDTH / 2), 750, "menu");
		...
		button_db_export.addEventListener(MouseEvent.CLICK, preferences_exportdb, false, 0, true);	
		...
		preferences_screen.addChild(button_db_export);
	...
	function preferences_exportdb(event: MouseEvent):Void {
		
		// Run export function
		var db_export_path = AppdataEntryLite.exportLiteDatabases();
		
		//Display info message
		export_litedb_info = new InfoText (language.gameinfo_texts.export_litedb + db_export_path,1);
		var export_litedb_info_button = export_litedb_info.getChildAt(1);
		export_litedb_info_button.addEventListener(MouseEvent.CLICK, toggleMessageExportLiteDB,false,0,true);
		preferences_screen.addChild(export_litedb_info);
				
	}
	
	
	public function toggleMessageExportLiteDB(event: MouseEvent):Void {
		preferences_screen.removeChild(export_litedb_info);
	}
	...
	public function RunResetUserdata(event: MouseEvent):Void {
		...
		if (database_availability == true) {
			...
			if (database_accessible == true) {
				...
				#if mobile
				reset_userdata_info = new InfoText (language.registration_text.inf_text5,1);
				#elseif desktop
				reset_userdata_info = new InfoText (language.registration_text.inf_text6,1);
				#end
		
				var reset_userdata_info_button_OK = reset_userdata_info.getChildAt(1);
				reset_userdata_info_button_OK.addEventListener(MouseEvent.CLICK, toggleMessageResetUserdata,false,0,true);
				preferences_screen.addChild(reset_userdata_info);
...
//%%%%%%%%%%INIT FUNCTION%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	function init() 
	{
		...
		// Check permissions if on mobile
		#if mobile
		var write_storage_permission = Permissions.hasPermission("android.permission.WRITE_EXTERNAL_STORAGE");
		var read_storage_permission = Permissions.hasPermission("android.permission.READ_EXTERNAL_STORAGE");
		var phone_state_permission = Permissions.hasPermission("android.permission.READ_PHONE_STATE");

		/*if(!write_storage_permission){
		...
		// Set round index to 0 as it will be increased to 1 in the newRound function
		_trial_ind = 0;
		// Set run index to 1 before updating it from local or remote database
		_run_ind = 1;
		// Initialize score
		_score = 0;
		...
		if (FileSystem.exists(login_savepath)) {
			...
			var database_availability = InternetConnection.isAvailable();
			if (database_availability == true) {
				// Pull current data from database and save if the database responds
				var database_accessible = DatabaseSync.SyncUserData(_id);
				if (database_accessible == true) {
				// Save local JSON userdata
				AppdataJSON.AppdataSave();
