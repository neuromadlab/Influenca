...
class DatabaseSync 
{
	...
	static public function CheckPasswordReset (mail_entry:String):Bool
	{
	...
	static public function UserRegistration ():Bool 
	{
	...
	static public function SyncUserData (identifier:Int):Bool
	{
		...
		if (subj_data == null) {
			...
			// First check if local database entries exist
			if (_run_ind == 1) {
			...
			// First check if local database entries are newer and only write if non-existent or older
			if (_run_ind < subj_data.run_ind + 1) {
			...
			//Update keycode if it is not set locally
			if (_keycode_set == 0) {
