...
class AppdataEntryLite
{
	...
	static public function exportLiteDatabases ():String
	{
		...
		// Create full path of SQLite databases
		var db_trial_name = _id + "_app_data_trial.db";
		var db_trial_path = Path.join([database_path, db_trial_name]);
		
		var db_run_name = _id + "_app_data_run.db";
		var db_run_path = Path.join([database_path, db_run_name]);
		
		// Set up export path
		#if desktop
			var export_path = Path.join([System.documentsDirectory, "Influenca_Data"]);
		#elseif mobile
			var androidexternalstorage = AndroidExternalStorage.getAndroidStoragePath();		
			var export_path = Path.join([androidexternalstorage, "Influenca_Data"]);
		#end
		if (!FileSystem.exists(export_path)) {
			 FileSystem.createDirectory(export_path);
		}
		
		// Generate full export paths
		var export_path_trial = Path.join([export_path, db_trial_name]);
		var export_path_run = Path.join([export_path, db_run_name]);
		
		if(FileSystem.exists(db_trial_path)) {
			// Copy database
			File.copy(db_trial_path, export_path_trial);
		}
		
		if(FileSystem.exists(db_run_path)) {
			// Copy database
			File.copy(db_run_path, export_path_run);
		}
